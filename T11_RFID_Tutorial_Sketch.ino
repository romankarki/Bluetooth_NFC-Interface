...
int RX_pin = 4; //for bluetooth module
int TX_pin = 2; //for bluetooth module

String BT_data;
String Arduino_data;

SoftwareSerial BTserial(RX_pin, TX_pin); // i.e 4 and 2
    ...
    if (Serial.available()){
      ...
      BTserial.println(Arduino_data);
		...
		//byte		sak;			// The SAK (Select acknowledge) byte returned from the PICC after successful selection.
         ...
         byte BT_content[16];
         BT_data.getBytes(BT_content,16);
         writeBlock(block, BT_content);//the blockcontent array is written into the card block
